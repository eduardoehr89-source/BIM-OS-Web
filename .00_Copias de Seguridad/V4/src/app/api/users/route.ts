...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { nombre, pin, tipo, rol, adminPin, companyId, newCompanyName, permisos } = body;
    ...
    if (!adminUser || adminUser.pin !== String(adminPin).trim()) {
    ...
    const user = await prisma.user.create({
      ...
      select: { id: true, nombre: true, tipo: true, rol: true, permisos: true, client: { select: { nombre: true } }, createdAt: true },
    });
...
export async function PATCH(request: Request) {
  ...
  try {
    const body = await request.json();
    const { id, nombre, tipo, rol, pin, adminPin, companyId, newCompanyName, permisos } = body;
