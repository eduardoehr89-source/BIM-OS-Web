...
export default async function DashboardPage() {
  ...
  if (isAdmin) {
    ...
    const ownersInfo = await prisma.user.findMany({
      where: { id: { in: tasksByOwner.map(g => g.ownerId).filter(Boolean) as string[] } },
      select: { id: true, nombre: true }
    });
    const ownerMap = Object.fromEntries(ownersInfo.map(o => [o.id, o.nombre]));
    workload = tasksByOwner.map(g => ({
      nombre: g.ownerId ? ownerMap[g.ownerId] : "Sin asignar",
      count: g._count.id
