...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (user.clientId) {
      try {
        ...
        clientUsers.forEach((u) => {
          if (u?.id) allowedUserIds.add(u.id);
        });
      } catch {
        /* ignorar bloque cliente */
      }
    }
    ...
    for (const p of allProjects) {
      ...
      p.sharedWith?.forEach((u) => {
        if (u?.id) allowedUserIds.add(u.id);
      });
    }

    const ids = Array.from(allowedUserIds).filter(Boolean);
    if (ids.length === 0) {
    ...
    const messagesRaw = await prisma.chatMessage.findMany({
      ...
      include: {
        sender: {
          select: { id: true, nombre: true, tipo: true, rol: true, client: { select: { nombre: true } } },
        },
        mentions: {
    ...
    const messages = messagesRaw.filter((m) => m.sender?.id);
  ...
  try {
    ...
    if (attachedFileId || attachedTaskId) {
      ...
      if (attachedFileId) {
        ...
        if (!file || file.isDeleted) return NextResponse.json({ error: "Archivo no encontrado" }, { status: 404 });
