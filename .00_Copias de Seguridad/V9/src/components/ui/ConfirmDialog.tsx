...
type ConfirmDialogProps = {
  ...
  requirePin?: boolean;
  /** Etiqueta del campo secreto (p. ej. «NIP» o «PIN de administrador»). */
  secretFieldLabel?: string;
  secretPlaceholder?: string;
  /** Si se define, el botón confirmar exige esta longitud exacta (p. ej. 4 para NIP). */
  secretExactLength?: number;
...
export function ConfirmDialog({
  ...
  requirePin = false,
  secretFieldLabel = "Tu PIN (Admin)",
  secretPlaceholder = "Confirma tu PIN",
  secretExactLength,
  onConfirm,
  onCancel,
}: ConfirmDialogProps) {
  ...
  const pinReady =
    !requirePin ||
    (secretExactLength != null
      ? localPin.trim().length === secretExactLength
      : Boolean(localPin.trim()));
          ...
          <div className="mt-4">
            <label className="block text-xs font-medium text-muted-foreground">
              ...
              <input
                ...
                inputMode="numeric"
                autoFocus
                maxLength={secretExactLength ?? undefined}
